...
if (__name__ == "__main__"):
    ...
    try:
        print('Starte ZIP Datei Erstellung...')
        rootdir = sys.path[0]
            ...
            if re.search("^(plugin|script|service|skin|repository|context)" , x) and not re.search('.zip', x):
                ...
                if "addon.xml" in filesinfoldertozip: 
                    ...
                    for elem in root.iter('addon'):
                        print('Addon zum packen %s %s Version: %s' % (x, elem.tag, elem.attrib['version']))
                ...
                for y in filesinfoldertozip: 
                    ...
                    if re.search("addon|changelog|icon|fanart", y):
                        ...
                        print('Kopiere %s in das Verzeichnis %s' % (y, zipsfolder))                
                # #check for and zip the folders
                print('Packe Datei %s und verschiebe sie in das Verzeichnis %s\n' % (x, zipsfolder))
                try:
                    zipfolder(zipfilenamefirstpart + version + zipfilenamelastpart, foldertozip, zipsfolder, x)
                    print('Packvorgang mit "Zipfolder" erfolgreich\n')
                except:
                    ...
                    try:
                        ...
                        print('Packvorgang mit "shutil" erfolgreich\n')
                    except Exception as e:
                        print('Cannot create zip file\nshutil %s\n' % e)
    except Exception as e:
        print('Cannot create or move the needed files\n%s' % e)
    print('Erstellung des Repositorys erfolgreich abgeschlossen !')
