...
def get_actor_window(window_type):
	...
	class DialogActorInfo(DialogBaseInfo, window_type):
		...
		def open_tvshow_dialog(self):
			selection = xbmcgui.Dialog().select(heading='Wählen Sie eine der Optionen', list=['Informationen zur Fernsehsendung anzeigen', 'Showschauspieler Fernsehshowauftritte'])
			if selection == 0:
