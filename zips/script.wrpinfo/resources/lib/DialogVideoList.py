...
def get_tmdb_window(window_type):
	...
	class DialogVideoList(DialogBaseList, window_type):
			...
			if selection == 1:
				if self.listitem.getProperty('TVShowTitle'):
					...
					if self.listitem.getProperty('dbid'):
						...
						if xbmcgui.Dialog().yesno('WRPinfo', '[B]%s[/B] zur Bibliothek hinzufügen?' % self.listitem.getProperty('TVShowTitle')):
							xbmc.executebuiltin('RunPlugin(plugin://plugin.video.wrp-metaplayer/tv/add_to_library/%s)' % tvdb_id)
							Utils.after_add(type='tv')
							Utils.notify(header='[B]%s[/B] zur Bibliothek hinzufügt' % self.listitem.getProperty('TVShowTitle'), message='Beenden und Aktualisierung durchführen', icon=self.listitem.getProperty('poster'), time=5000, sound=False)
				else:
					if self.listitem.getProperty('dbid'):
						if xbmcgui.Dialog().yesno('WRPinfo', '[B]%s[/B] aus der Bibliothek entfernen?' % self.listitem.getProperty('title')):
							...
							if os.path.exists(xbmc.translatePath('%s%s/' % (MovieLibrary, imdb_id))):
								...
								Utils.notify(header='[B]%s[/B]' % self.listitem.getProperty('title'), message='aus der Bibliothek entfernt', icon=self.listitem.getProperty('poster'), time=5000, sound=False)
								xbmc.sleep(250)
								self.update(force_update=True)
								self.getControl(500).selectItem(self.position)
					else:
						if xbmcgui.Dialog().yesno('WRPinfo', '[B]%s[/B] zur Bibliothek hinzufügen?' % self.listitem.getProperty('title')):
							xbmc.executebuiltin('RunPlugin(plugin://plugin.video.wrp-metaplayer/movies/add_to_library/tmdb/%s)' % item_id)
							Utils.after_add(type='movie')
							Utils.notify(header='[B]%s[/B] zur Bibliothek hinzufügt' % self.listitem.getProperty('title'), message='Beenden und Aktualisierung durchführen', icon=self.listitem.getProperty('poster'), time=5000, sound=False)
			if selection == 2:
				if self.listitem.getProperty('TVShowTitle'):
		...
		def set_actor_filter(self):
			result = xbmcgui.Dialog().input(heading='Suchbegriff eingeben', type=xbmcgui.INPUT_ALPHANUM)
