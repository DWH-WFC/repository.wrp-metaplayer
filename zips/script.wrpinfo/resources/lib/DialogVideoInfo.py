...
def get_movie_window(window_type):
	...
	class DialogVideoInfo(DialogBaseInfo, window_type):
		...
		def add_movie_to_library(self):
			...
			if xbmcgui.Dialog().yesno('WRPinfo', '[B]%s[/B] zur Bibliothek hinzufügen?' % self.info['title']):
				...
				Utils.notify(header='[B]%s[/B] zur Bibliothek hinzugefügt' % self.info['title'], message='Beenden und Aktualisierung starten', icon=self.info['poster'], time=5000, sound=False)

		@ch.click(19)
		def remove_movie_from_library(self):
			if not xbmc.getCondVisibility('System.HasAddon(plugin.video.wrp-metaplayer)'):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.wrp-metaplayer/setup/total)')
			if xbmcgui.Dialog().yesno('WRPinfo', '[B]%s[/B] aus Bibliothek entfernen?' % self.info['title']):
				if os.path.exists(xbmc.translatePath('%s%s/' % (Utils.OPENMETA_MOVIE_FOLDER, self.info['imdb_id']))):
					...
					Utils.after_add(type='movie')
					Utils.notify(header='[B]%s[/B] aus Bibliothek entfernt' % self.info.get('title', ''), message='Beenden und Aktualisierung starten', icon=self.info['poster'], time=5000, sound=False)
