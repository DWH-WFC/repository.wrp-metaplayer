	...
	class DialogTVShowInfo(DialogBaseInfo, window_type):
		...
		def right_click_similar(self):
			...
			if self.listitem.getProperty('dbid'):
				listitems += ['Aus der Bibliothek entfernen']
			else:
		...
		def add_tvshow_to_library(self):
			...
			if xbmcgui.Dialog().yesno('WRPinfo', '[B]%s[/B] zur Bibliothek hinzufügen?' % self.info['TVShowTitle']):
				...
				Utils.notify(header='[B]%s[/B] zur Bibliothek hinzugefügt' % self.info['TVShowTitle'], message='Beenden und Aktualisierung starten', icon=self.info['poster'], time=5000, sound=False)

		@ch.click(21)
		def remove_tvshow_from_library(self):
			if not xbmc.getCondVisibility('System.HasAddon(plugin.video.wrp-metaplayer)'):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.wrp-metaplayer/setup/total)')
			if xbmcgui.Dialog().yesno('WRPinfo', '[B]%s[/B] aus der Bibliothek entfernen?' % self.info['TVShowTitle']):
				if os.path.exists(xbmc.translatePath('%s%s/' % (Utils.OPENMETA_TV_FOLDER, self.info['tvdb_id']))):
					...
					Utils.after_add(type='tv')
					Utils.notify(header='[B]%s[/B] wurde aus der Bibliothek entfernt' % self.info['TVShowTitle'], message='Beenden und Aktualisierung starten', icon=self.info['poster'], time=5000, sound=False)
